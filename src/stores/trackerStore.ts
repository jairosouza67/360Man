...
export const useTrackerStore = create<TrackerStore>((set, get) => ({
  ...
  loadPlans: async (userId: string) => {
    try {
      // NOTE: Firestore requires a composite index when combining a where() on
      // `userId` with an orderBy() on `createdAt`. If you see a runtime error
      // like "The query requires an index", create the index in Firebase
      // Console using the link below (this link is generated by the SDK when
      // the query fails and can be found in the browser console error):
      //
      // https://console.firebase.google.com/v1/r/project/respect-pill/firestore/indexes?create_composite=Ckpwcm9qZWN0cy9yZXNwZWN0LXBpbGwvZGF0YWJhc2VzLyhkZWZhdWx0KS9jb2xsZWN0aW9uR3JvdXBzL3BsYW5zL2luZGV4ZXMvXxABGgoKBnVzZXJJZBABGg0KCWNyZWF0ZWRBdBACGgwKCF9fbmFtZV9fEAI
      //
      // Temporary workaround: remove orderBy() or sort client-side after fetching.
      ...
      // If the query requires an index the SDK usually provides a link in the
      // error message. Log a clearer message for developers to follow the
      // console-provided link to create the composite index.
      const msg = error?.message || String(error);
      if (msg.includes('requires an index') || msg.includes('create_composite')) {
        console.error('Firestore composite index required for query on `plans`. Create it here:');
        console.error('https://console.firebase.google.com/v1/r/project/respect-pill/firestore/indexes');
      }
    }
  },
